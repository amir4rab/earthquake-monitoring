...
import { getCachedLatestData, setLatestEarthquakeDataToCache, delCachedLatestData } from '@/redis';
...
const getLatestData: GetLatestData = async (skipCache= false) => {
  try {
    if ( !skipCache ) {
      ...
      if ( cachedStateData !== null ) return cachedStateData as ExtendedEarthquakeArray;
    } else {
    ...
    await setLatestEarthquakeDataToCache(latestSortedData as ExtendedEarthquakeArray);
