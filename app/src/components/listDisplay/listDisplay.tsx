...
// mantine
import { Box, Table, Title, useMantineTheme } from '@mantine/core';
...
const ListDisplay = ({ title, content, namespace, searchable=false }: Props) => {
  ...
  const filteredArray = useMemo(() => { // filters the content to find the included items
    if ( filesAreValid.current !== true || searchable === false || searchQuery.length === 0 ) return content.rows;
    try {
  ...
  useEffect(() => {
    ...
    if ( biggestIndex > content.header.length - 1 ) {
      console.error(`Couldn't found index "${biggestIndex}", please recheck your searchable fields`);
    ...
    <Box>
      ...
      <Box sx={{ width: '100%', overflowX: 'auto' }}>
        <Table striped highlightOnHover>
          <thead>
            <tr>
              { content.header.map(i => (<th style={{ textAlign: dir === 'rtl' ? 'right' : 'left' }} key={ i }>{ t(i) }</th>))  }
            </tr>
          </thead>
          <tbody>
            {
              filteredArray.map(({ id, items }) => (
                <tr key={ id }>
                  { items.map(i => <td key={ i.key }>{ i.el }</td>) }
                </tr>
              ))
            }
          </tbody>
