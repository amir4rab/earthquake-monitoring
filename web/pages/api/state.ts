...
const handler = async (
  ...
  try {
    ...
    if ( typeof req.query?.page !== 'string' || typeof req.query?.state !== 'string' ) return res.status(400).json({ 
    ...
    // verifying charset
    if ( !regExp.test(req.query.page) || !regExp.test(req.query.state) ) return res.status(406).json({ 
