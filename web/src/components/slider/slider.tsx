...
import { Group, BoxProps, Box, Text } from '@mantine/core';
...
type MarkType = { value: number, label: string, i18nKey?: string } | { value: number, label?: undefined, i18nKey: string };
...
const Slider = ({ boxStyles= {}, onChange, value, marks= [], label }: SliderProps ) => {
          ...
          <div className={ classes.marksWrapper }>
            {
              marks.map(({ label, value, i18nKey }) => (
                <Text 
                  ...
                  style={{
                    left: `${value}%`, 
                    transform: value === 100 ? 'translate(-100%, -50%)' : value === 0 ? 'translate(0%, -50%)' : 'translate(-50%, -50%)' 
                  }} 
