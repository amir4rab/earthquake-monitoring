...
import { getCachedSatesEarthquakeDataById, setEarthquakeDataToCache, delCachedStateDataById } from '@/redis';
...
const getStateData: (v: GetStateDataProps) => Promise<GetStateDataReturn> = async ({ stateId, skipCache= false, page }) => {
  try {
    if ( !skipCache ) {
      const cachedStateData = await getCachedSatesEarthquakeDataById(stateId);
    
      if ( cachedStateData !== null ) return ({
        latestEarthquakesArr: cachedStateData[page] as ExtendedEarthquakeArray,
    ...
    const { latestEarthquakesArr } = await GetStatesDataFromPostgres({ stateId, pageSize: 25 * 5 });
    ...
    for ( let i = 0; i < totalPages; i++ ) {
      slicedStateData.push(latestEarthquakesArr.slice(i * pageSize, (i+1)*pageSize) as ExtendedEarthquakeArray);
    };
