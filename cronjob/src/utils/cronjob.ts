...
const cronjob = async ({ verbose= false, revalidateInDev= false }: cronjobProps) => {
  try {
    ...
    if ( process.env.NODE_ENV === 'production' || revalidateInDev ) {
      ...
      if ( typeof apiRoute !== 'string' || typeof apiKey !== 'string' ) {
        throw new Error('Please set following env files: "NEXT_JS_API_ROUTE", "NEXT_JS_API_KEY"!');
      }
