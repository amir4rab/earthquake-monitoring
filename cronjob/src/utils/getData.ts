...
const getData = async (verbose = false) => {
  try {
    ...
    enJson.forEach((i, index) => {
      ...
      if (stateCode === null && verbose)
        console.error(`⚠️ Failed to find state code for "${stateEn}"`);
    });
